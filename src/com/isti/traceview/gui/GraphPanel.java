...
public class GraphPanel extends JPanel implements Printable, MouseInputListener, Observer {
		...
		if(!paintNow){
			...
			if (initialPaint ||  forceRepaint || !mouseRepaint || ChannelView.tooltipVisible) {
				...
				if (previousMouseX >= 0 && previousMouseY >= 0) {
					// Erasing cursor
					if (showBigCursor) {
						if (cvMouseMoved) {	// only erase cursor if in ChannelView panel
				...
				if (mouseX > infoPanelWidth && mouseY < getHeight() - southPanel.getHeight()) {
					// Drawing cursor
					if (showBigCursor) {
						if (cvMouseMoved) {	// only draw cursor if in ChannelView panel
	...
	public void mouseMoved(MouseEvent e) {
		if ((button != MouseEvent.NOBUTTON) && (e.isControlDown() || e.isShiftDown())) {
			...
			if (cvMouseMoved) {	// only repaint and update when moving in/between ChannelView panels
	...
	public void mouseEntered(MouseEvent e) {
		if (cvMouseEntered) {
			System.out.println("ChannelView.mouseEntered(e) --> repaint()\n");
			forceRepaint = false;
			repaint();
		} else {
			System.out.println("GraphPanel.mouseEntered(e)\n");
			forceRepaint = false;
	...
	public void mouseExited(MouseEvent e) {
		if (mouseX != -1 || mouseY != -1) {
			if (cvMouseExited) {
				System.out.println("ChannelView.mouseExited(e) --> repaint()\n");
			} else {
