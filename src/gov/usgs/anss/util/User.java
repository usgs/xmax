...
/**
 * This class encapsulate the notion of a user of the system per the inventory
 * system.
 * 
...
public class User {
	...
	public static String UserString() {
		return "user=" + username + " master=" + master + "name=" + name;
	}
	 ...
	 * 
	 * @param roles
	 *            A list of single character roles
	...
	/*
	 * Like User(Connection, String, String) without the password checking. All
	 * it does is set the password, role, etc.
	 */
	public User(Connection C, String user) throws SQLException {
		...
		try {
			...
			Util.prta("  ****** User "
					+ user
					+ " is not in anss.person assume master since roles are not set up");
	...
	public User(String database, Connection C, String user) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM " + database
					+ ".person" + " WHERE person = " + Util.sqlEscape(user));
			rs.next();
			userID = Util.getInt(rs, "ID");
			username = Util.getString(rs, "Person");
			master = Util.getString(rs, "Master");
			...
			allowedRoles = "M";
			role = allowedRoles;
			Util.prta("  ****** User " + user + " is not in " + database
					+ ".person assume master since roles are not set up");
			// Util.SQLErrorPrint(E,"SQL User failed");
			// E.printStackTrace();
		}
	}
		...
		try {
			...
			if (password.compareTo(passReturned) != 0) {
				...
				Util.prt("not equal|" + password + "|" + passReturned + "|"
						+ password.length() + passReturned.length() + " ct:"
						+ password.compareTo(passReturned));
