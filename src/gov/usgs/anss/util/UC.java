...
public class UC
{
	...
	public static String RTS_SERVER_NODE = "gacq1"; // Where the RTSServer
													// program is running
	...
	public static void setFedCtr() {
		if (Util.getProperty("MySQLServer").equals(FEDERAL_CENTER_IP_ADDR)) {
			...
			RTS_SERVER_NODE = "136.177.23.85"; // Where the RTSServer program is
												// running
	...
	public static String JDBCDatabase() {
		...
		return "jdbc:mysql://" + JDBCHost() + "/" + JDBCCatalog() + "?user="
				+ DEFAULT_USER + "&password=" + DEFAULT_PASSWORD;
	}
	...
	/**
	 * Return the Connection previously saved with
