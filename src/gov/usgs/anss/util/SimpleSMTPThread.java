...
/**
 * simpleSMTP.java - This class is used to send a simple mail message (no
 * attachements, etc. just subject and text body) to a SMTP server. The main
 * thread does the sending and an inner class reads data back from the smtp
 * server. A StringBuffer is maintained with details of the session so a user
 * could inspect or use it for debugging.
 * 
...
public class SimpleSMTPThread extends Thread {
	public static String[] dow = { "NaN", "Sun", "Mon", "Tue", "Wed", "Thu",
			"Fri", "Sat" };
	 ...
	 * thread which sends the message and spawns an inner class which monitors
	 * and records the responses of the SMTP server. If subject line contains
	 * 'DBG' then debugging output of the thread will be turned on.
	 * 
	 * @param serverin
	 *            The SMTP server machine to use (port 25)
	 * @param to_in
	 ...
	 */
	public SimpleSMTPThread(String serverin, String to_in, String from_in,
			String subject_in, String body_in) {
		dbg = debugOverride;
		if (serverin == null || serverin.equals(""))
			server = Util.getProperty("SMTPServer");
		else
			server = serverin;
		if (subject_in.indexOf("DBG") >= 0)
			dbg = true;
		to = to_in.trim();
		from = from_in.trim();
		...
		running = true;
		if (dbg)
			Util.prt("SMTPT: serv=" + server + "HostIP="
					+ Util.getProperty("HostIP") + " to " + to + " from "
					+ from + "\nsub=" + subject);
		successful = false;
	 ...
	 * server in the normal manner and monitors responses of the server via a
	 * GetSession inner class object. Beware per
	 * http://pobox.com/~djb/docs/smtplf.html sending linefeeds without
	 * preceding carriage returns can result in rejections of mail by some
	 * servers per a RFP spec. We spent much time running this down!
	 */
	@Override
	public void run() {
		...
		try {
			...
			try {
				...
				if (e.getMessage() != null) {
					if (e.getMessage().indexOf("Connection timed out") >= 0) {
						Util.prta("SMTPT: 1st Connection timed out to server="
								+ server + " to " + to + " subj=" + subject
								+ " "
								+ (System.currentTimeMillis() - connectTime));
						connectTime = System.currentTimeMillis();
						s = new Socket(server, 25);
					} else {
						Util.prta("SMTPT: IOError" + e + " server=" + server
								+ " to " + to + " subj=" + subject + " "
								+ (System.currentTimeMillis() - connectTime));
						SendEvent.debugEvent("SMTPThrErr", "IOError e=" + e,
								this);
						return;
					}
				} else {
					Util.prt("SMTPT: null IOexception=" + e.toString()
							+ " server=" + server + " to " + to + " subj="
							+ subject);
					err.append("SimpleSMTPThread exception : " + e.toString()
							+ " server=" + server);
					SendEvent.debugEvent("SMTPThrErr", "IOException e=" + e,
							this);
					running = false;
					return;
				}
			...
			// get the input stream from the socket and wrap it in a
			// BufferedReader
			InputStream in = s.getInputStream();
			bin = new BufferedReader(new InputStreamReader(in));
			GetSession get = new GetSession(bin, err); // Thread listening for
														// input!
			...
			if (Util.getProperty("HostIP") != null)
				if (Util.getProperty("HostIP").length() > 7)
					str = "HELO " + Util.getProperty("HostIP");

			if (dbg)
				System.out.println("SMTPT:" + str); // display what we're
													// sending
			err.append("Send:" + str + "\n");
			...
			if (!get.chkFor("Sender", "ok")) {
				...
				if (!get.chkFor("Recipient", "ok")) {
					...
					successful = false;
					if (dbg)
						System.out.println("SMTPT:" + "No recipient o.k.");
				} else {
					...
					if (!get.chkFor("354")) {
						...
						err.append("Send:SUBJECT:" + Util.asctime() + " "
								+ subject + "\n");
						...
						String d = "Date:"
								...
								+ ", "
								+ df2.format(now.get(Calendar.DAY_OF_MONTH))
								+ " "
								+ months[now.get(Calendar.MONTH)]
								+ " "
								+ now.get(Calendar.YEAR)
								+ " "
								+ df2.format(now.get(Calendar.HOUR_OF_DAY))
								+ ":"
								+ df2.format(now.get(Calendar.MINUTE))
								+ ":"
						...
						/*
						 * pout.write(("Date:"+d.substring(0,3)+", "+d.substring(
						 * 8,10)+" "+
						 * d.substring(4,7)+" "+d.substring(24,28)+" "
						 * +d.substring(11,19)+" -0"+
						 * -now.get(GregorianCalendar.
						 * ZONE_OFFSET)/36000+"\r\n").getBytes());
						 */
			...
			if (dbg || !successful)
				Util.prta("SMTPT:----- Error list --------\n" + err.toString());
			if (dbg || !successful)
				Util.prta("SMTPT:------Session --------- \n" + get.getSession());
			...
			if (e.getMessage() != null) {
				if (e.getMessage().indexOf("Connection timed out") >= 0) {
					Util.prta("SMTPT: 2nd Connection timed out to server="
							+ server + " to " + to + " subj=" + subject + " "
							+ (System.currentTimeMillis() - connectTime));
					SendEvent.debugEvent("SMTPThrErr",
							"Connection2 timed out to " + server, this);
				} else {
					Util.prta("SMTPT: IOError=" + e + " to server=" + server
							+ " to " + to + " subj=" + subject + " "
							+ (System.currentTimeMillis() - connectTime));
					SendEvent.debugEvent("SMTPThrErr", "IOerror2=" + e
							+ " server=" + server, this);
				}
			} else {
				Util.prt("SMTPT: exception2=" + e.toString() + " server="
						+ server);
				err.append("SimpleSMTPThread exception : " + e.toString()
						+ " server=" + server);
	...
	/**
	 * wait for the SMTP mail session to complete. This will sleep the caller
	 * until the SMTP session has completed. At the time of the return the
	 * session text will be complete
	 */
	...
	/**
	 * Return wether the mail message appeared to go out successfully to the
	 * server
	 * 
	...
	/**
	 * Return a String with a record of the SMTP session. Normally only used for
	 * debugging the send.
	 * 
	...
	class GetSession extends Thread {
		...
		/**
		 * look for two string on the last line (like "recipient", "ok") case
		 * insensitive.
		 * 
		 * @param str1
		 *            first string to look for
		 * @param str2
		 *            2nd string to look for
		...
		/**
		 * conveniently clear the receive buffer input which shutsdown the
		 * thread
		 */
		public void setBin(BufferedReader b) {
			bin = b;
		}
	 ...
	 * String killpages = Util.getProperty("KillPages"); if(killpages != null) {
	 * Util.prta("SMTPT: KillPages is "+killpages); if(killpages.equals("true"))
	 * { Util.prta("SMTPT: Pagekilled subj="+subject+" body="+body); return
	 * email("ketchum","PAGE KILLED:"+subject, body); } } String override =
	 * Util.getProperty("KetchumPageOverride"); if(override != null) {
	 ...
	 * body.substring(0,Math.min(body.length(),5))); } } return
	 * email("3035205840@vtext.com"
	 * ,subject.substring(0,Math.min(subject.length(),100)),
	...
	public static void main(String[] args) {
		...
		if (subject != null) {
			...
			try {
				...
				for (int j = istart; j < args.length; j++) {
					SimpleSMTPThread thr = SimpleSMTPThread.email(args[j],
							subject, sb.toString());
					thr.waitForDone();
					if (thr.wasSuccessful()) {
						Util.prt("Your email was sent to " + args[j]);
					} else {
						Util.prt("Email failed to " + args[j] + " "
								+ thr.getSendMailError());
					}
		...
		/*
		 * SimpleSMTPThread ab =
		 * email("ejhaug07@att.blackberry.net","Test page to Eric"
		 * ,"This message had the Date changed to two digit year.\n");
		 * ab.waitForDone(); Util.prt("ejh="+ab.getSendMailError());
		 */

		SimpleSMTPThread a = email("3035205840@vtext.com", "DBG:Test page "
				+ Util.asctime(), "body1");
		a.waitForDone();
		...
		a.waitForDone();
		Util.prta("1st=" + a.getSendMailError());
		a = email("ketchum@usgs.gov",
				"DBG:subject with usgs.gov " + Util.asctime(), "body2");
		a.waitForDone();
		Util.prta("2ND=" + a.getSendMailError());
		// a = pageDave("Test Paging subject "+Util.asctime(),
		// "Test Page BODY");
		// a.waitForDone();
