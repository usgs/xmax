 ...
 * stderr and stdout every 0.1 seconds and exiting when it discovers the
 * subprocess has exited. The waitFor() not only waits for the subprocess to
 * exit, but insures the run() has exited insuring the stdout and stdin buffers
...
public class Subprocess extends Thread {
	...
	final StringBuffer errText = new StringBuffer(200); // Build the error
														// output here
	final StringBuffer outText = new StringBuffer(1000); // Build the stdout
															// here
	...
	/**
	 * This is implemented as a Thread so that it will keep reading any stderr
	 * or stdout input into the StringBuffers so that they cannot block the
	 * operating system from running the subprocess. Read any input each 1/10th
	 * second and exits when the subprocess has exited.
	 */
	@Override
	public void run() {
		...
		while (val == -99) {
			...
			getInput();
			/*
			 * it is possible but unlikely that input comes in after being
			 * emptied abovebut then finding that the subprocess has exited.
			 */
			if (loop++ % 2 == 0)
				val = exitValue();
	 ...
	 * the subprocess is done so be careful that the subprocess does not dead
	 * lock or run continuously.
	 * 
	 * @return Exit value of process, zero is success.
	 * @throws InterruptedException
	 *             Means wait did not complete but was interrupted
	 */
	 ...
	 * running this should return -99 to indicate it is still running and so can
	 * be used to test for the process still running without blocking the
	 * thread's execution
	 * 
	 * @return The exit status of the subprocess or -99 if the subprocess is
	 *         still running
	 */
	...
	/**
	 * return the output of the subprocess as a string. If the subprocess is
	 * still running, it may return some portion of the stdout to this point,
	 * but will not block.
	 * 
	 ...
	 * return the output of the subprocess as a string. If the subprocess is
	 * still running, it may return some portion of the stdout to this point,
	 * but will not block. if flag is true, clear the output buffer
	 * 
	 * @param clear
	 *            If true, clear the output buffer after getting the string
	 * @return String with the contents of stdout
	 */
	 ...
	 * 
	 * @param clear
	 *            If true, clear the text field after returning it
	 * @return String with contents of stderr
	 */
