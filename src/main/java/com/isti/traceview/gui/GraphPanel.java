 ...
 * @author Max Kokoulin
 */
public class GraphPanel extends JPanel implements Printable, MouseInputListener,
  ...
  static {
    ...
    hiddenCursor = Toolkit.getDefaultToolkit()
        .createCustomCursor(image, new Point(0, 0), "HIDDEN_CURSOR");
  }
  ...
  public GraphPanel(boolean showTimePanel) {
    ...
    setListener(new PropertyChangeSupport(this));
  ...
  public void setTimeRange(TimeInterval ti) {
    ...
    TimeInterval oldTimeRange = this.timeRange;
    this.timeRange = ti;
    ...
    getListener().firePropertyChange("time range", oldTimeRange, ti);
    forceRepaint();
  }

  /**
   * If true, graph panel itself changes time range after data set changing to show all loaded data.
   * If false, given time range used.
   *
   * @param value Flag if graphPanel should manage its TimeInterval itself
   */
  ...
  /**
   * If true, graph panel selects blue area while mouse dragging and calls time range or value range
   * changing after mouse releasing.
   *
  ...
  /**
   * Getter of the property <tt>channelShowSet</tt>. Returns list of views for current page without
   * influence of selection commands, like select or overlay
   *
  ...
  public void setChannelShowSet(List<PlotDataProvider> channels) {
    ...
    if (channels != null) {
      ...
      if (!TraceView.getConfiguration().getMergeLocations()) {
        ...
        for (PlotDataProvider channel : channels) {
          ...
          if (prevChannel != null
              ...
              || !prevChannel.getStation().getName().equals(channel.getStation().getName())
              || !prevChannel
      ...
      if (overlay) {
        overlay = false;
        getListener().firePropertyChange("overlay", true, false);
      }
      if (select) {
        select = false;
        getListener().firePropertyChange("select", true, false);
      }
      if (rotation != null) {
        rotation = null;
        getListener().firePropertyChange("rotation", true, false);
      }
      repaint();  // why repaint when adding channels to Graph?
    }
    getListener().firePropertyChange("channels", null, channels);
  }
  ...
  public void clearChannelShowSet() {
    for (ChannelView cv : channelShowSet) {
      for (PlotDataProvider channel : cv.getPlotDataProviders()) {
        channel.removePropertyChangeListener(cv);
      }
    }
   ...
   *
   * @return Count of display units used to determine which subset of loaded traced should be shown
   */
   ...
   *
   * @param unitsShowCount Count of display units used to determine which subset of loaded traced
   * should be shown
   */
  ...
  public void setScaleMode(IScaleModeState scaleMode) {
    IScaleModeState oldMode = this.scaleMode;
    ...
    getListener().firePropertyChange("scale mode", oldMode, scaleMode);
    repaint();
  }
  ...
  public void setColorMode(IColorModeState colorMode) {
    IColorModeState oldColorMode = this.colorMode;
    this.colorMode = colorMode;
    getListener().firePropertyChange("color mode", oldColorMode, colorMode);
    repaint();
  }
  ...
  public void setMeanState(IMeanState meanState) {
    IMeanState oldMeanState = this.meanState;
    this.meanState = meanState;
    // returns XHair mode to all data after scale mode switching
    // manualValueMax = Integer.MIN_VALUE;
    // manualValueMin = Integer.MAX_VALUE;
    getListener().firePropertyChange("mean state", oldMeanState, meanState);
    repaint();
  }
  ...
  public void setOffsetState(IOffsetState offsetState) {
    IOffsetState oldOffsetState = this.offsetState;
    this.offsetState = offsetState;
    getListener().firePropertyChange("offset state", oldOffsetState, offsetState);
    repaint();
  }
  ...
  public void setPickState(boolean pickState) {
    boolean oldPickState = this.pickState;
    ...
    if (oldPickState != pickState) {
      getListener().firePropertyChange("pick state", oldPickState, pickState);
    }
    repaint();
  }
  ...
  public void setRemoveGainState(RemoveGain gain) {
    ...
    overlay = false;
    this.gain = gain;
    getListener().firePropertyChange("gain state", oldGain, gain);
    forceRepaint();
  }
   ...
   *
   * @return current overlay mode. If overlay mode turned on, all traces loaded in graph panel will
  ...
  /**
   * Switch overlay mode on/off. If overlay mode turned on, all traces loaded in graph panel will be
   * shown in one shared ChannelView
   */
  public void overlay() {
    if (overlay) {
      drawAreaPanel.removeAll();
    ...
    getListener().firePropertyChange("overlay state", !overlay, overlay);
  ...
  /**
   * Switch selection mode on/off. If selection mode turned on, will be shown only ChannelViews with
   * selected selection checkboxes.
   */
  public void select(Boolean undo) {
    ...
    getListener().firePropertyChange("select state", !select, select);
    getListener().firePropertyChange("overlay state", !overlay, overlay);
  ...
  /**
   * Sets earthquakes and phase names to draw on the graphs. Will be drawn only phases which satisfy
   * both sets.
   *
   * @param earthquakes set of earthquakes
   * @param phases set of phase names
   */
  ...
  public void addPropertyChangeListener(PropertyChangeListener o) {
    getListener().addPropertyChangeListener(o);
  }
   ...
   * @param o the o
   */
  public void deleteObserver(PropertyChangeListener o) {
    getListener().removePropertyChangeListener(o);
  }
  ...
  public void paint(Graphics g) {
    if (!paintNow) {
      ...
      if (initialPaint || forceRepaint || !mouseRepaint || ChannelView.tooltipVisible) {
        ...
        paintSelection(g, selectedAreaXbegin, selectedAreaXend, selectedAreaYbegin,
            selectedAreaYend, "Drawing");
        ...
        paintSelection(g, previousSelectedAreaXbegin, previousSelectedAreaXend,
            previousSelectedAreaYbegin, previousSelectedAreaYend, "Erasing");
   ...
   */
  private void paintSelection(Graphics g, long Xbegin, long Xend, double Ybegin, double Yend,
      String message) {
    int infoPanelWidth = channelViewFactory.getInfoAreaWidth();
    if (Xbegin != Long.MAX_VALUE && Xend != Long.MIN_VALUE && mouseSelectionEnabled) {
      ...
      if (Xend > Xbegin) {
        ...
        int leftPos = begPos >= 0 ? begPos + infoPanelWidth + getInsets().left
            : infoPanelWidth + getInsets().left;
        int rightPos = begPos >= 0 ? getXposition(Xend) - getXposition(Xbegin)
            : getXposition(Xend) - getXposition(Xbegin) + begPos;
        g.fillRect(leftPos, 0, rightPos, getHeight());
      } else {
        int begPos = getXposition(Xend);
        int leftPos = begPos >= 0 ? begPos + infoPanelWidth + getInsets().left
            : infoPanelWidth + getInsets().left;
        int rightPos = begPos >= 0 ? getXposition(Xbegin) - getXposition(Xend)
            : getXposition(Xbegin) - getXposition(Xend) + begPos;
        g.fillRect(leftPos, 0, rightPos, getHeight());
      }
    }
    if (!Double.isNaN(Ybegin) && !Double.isNaN(Yend)) {
      ...
      if (Yend > Ybegin) {
        g.fillRect(infoPanelWidth, getScaleMode().getY(Yend), getWidth(),
            getScaleMode().getY(Ybegin)
                - getScaleMode().getY(Yend));
      } else {
        g.fillRect(infoPanelWidth, getScaleMode().getY(Ybegin), getWidth(),
            getScaleMode().getY(Yend)
                - getScaleMode().getY(Ybegin));
      }
    }
  ...
  public long getTime(int x) {
    ...
    Insets i = getInsets();
    double sr =
        (double) getTimeRange().getDuration() / (getWidth() - i.left - i.right - channelViewFactory
            .getInfoAreaWidth());
  ...
  public int getXposition(long date) {
		if (getTimeRange() == null) {
			...
			return (int) (
					(getWidth() - channelViewFactory.getInfoAreaWidth() - getInsets().left
							- getInsets().right)
							* (double) (date - getTimeRange().getStart()) / (double) getTimeRange()
							.getDuration());
		}
  }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    ...
    logger.debug(this + ": update request from " + evt);
...
// jfreechart:  DateAxis.draw( Graphics2D, double cursor, Rectangle2D plotArea, Rectangle2D drawArea, ...)
        ...
        axis.draw((Graphics2D) g, 0,
            new Rectangle(infoPanelWidth + getInsets().left, 0, getWidth() - infoPanelWidth + 2,
                getHeight()),
            new Rectangle(infoPanelWidth + getInsets().left, 0, getWidth() - infoPanelWidth + 2,
                10),
  ...
  static class TimeInfoPanel extends JPanel {
    ...
    public void update(TimeInterval ti) {
      ...
      if (ti != null) {
        start.setText(TimeInterval
            .formatDate(ti.getStartTime(), TimeInterval.DateFormatType.DATE_FORMAT_NORMAL));
        duration.setText(ti.convert());
        end.setText(TimeInterval
            .formatDate(ti.getEndTime(), TimeInterval.DateFormatType.DATE_FORMAT_NORMAL));
      }
    }
  }
...
//infoPanel.setBorder(BorderFactory.createLineBorder(Color.BLACK,2));
  ...
  public void setListener(PropertyChangeSupport listener) {
